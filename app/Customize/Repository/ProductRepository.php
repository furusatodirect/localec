...
//            ->setParameter('DeliveredOnTo', new \DateTime())
        ...
        if ((!empty($searchData['category_ids']) && $searchData['category_ids'])) {
            foreach ($searchData['category_ids'] as $category_id) {
                $Categories = array_merge($Categories ?? [], $category_id->getSelfAndDescendants());
            }
        }
        ...
        if (isset($searchData['operator_name']) && StringUtil::isNotBlank($searchData['operator_name'])) {
            $qb
                ->andWhere('NORMALIZE(p.operator_name) LIKE NORMALIZE(:operator_name)')
                ->setParameter('operator_name', $searchData['operator_name'])
            ;
        }
...
//        }
        ...
        //　売れ筋順
            ...
            $qb->setParameter('current_date', $currentDate);

            $select = '(SELECT CASE WHEN SUM(oi.quantity) IS NULL THEN 0 ELSE SUM(oi.quantity) END FROM \Eccube\Entity\OrderItem AS oi
                    LEFT JOIN oi.Order AS o
                    WHERE
                     oi.Product=p.id
                     AND o.OrderStatus = true
                     AND o.order_date >= :start_date AND o.order_date < :current_date) AS HIDDEN buy_quantity';
...
//            ->setParameter('DeliveredOnTo', new \DateTime())
        ...
        if (isset($searchData['operator']) && StringUtil::isNotBlank($searchData['operator'])) {
            $qb
                ->andWhere('NORMALIZE(p.operator_name) LIKE NORMALIZE(:operator_name)')
                ->setParameter('operator_name', $searchData['operator_name'])
            ;
