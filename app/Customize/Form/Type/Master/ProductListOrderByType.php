...
class ProductListOrderByType extends AbstractType
{
    ...
    public function buildForm(FormBuilderInterface $builder, array $options)
    {
        ...
        $builder->addEventListener(FormEvents::PRE_SUBMIT, function (FormEvent $event) {
             ...
             */
            $values = [];
            foreach($options['choice_loader']->loadChoiceList()->getChoices() as $productOrderBy) {
                $values[] = $productOrderBy->getId();
            }
//            $values = $options['choice_loader']->loadChoiceList()->getValues();
            log_info(__METHOD__, $values);
            log_info(__METHOD__, [$event->getData()]);
